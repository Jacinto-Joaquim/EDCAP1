...
const AdminLoginPage = () => {
  ...
  const { login, loading, isAuthenticated, authInitialized, user } = useAuth();
  ...
  useEffect(() => {
    // Só verificar autenticação após inicialização completa do contexto
    if (!authInitialized) {
      console.log("Aguardando inicialização do contexto de autenticação...");
      return;
    }
    
    if (isAuthenticated && user) {
      console.log("Usuário já autenticado, redirecionando para dashboard:", user.email);
      navigate('/painel/dashboard', { replace: true });
    }
  }, [isAuthenticated, user, navigate, authInitialized]);

  // Efeito para navegação após login bem-sucedido
  useEffect(() => {
    if (loginAttempted && isAuthenticated && user && !loading) {
      console.log("Login bem-sucedido, redirecionando para:", from);
      
      // Usar um timeout maior para garantir que todos os estados estejam atualizados
  ...
  }, [loginAttempted, isAuthenticated, user, loading, navigate, from]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!success) {
        console.log("Login falhou, resetando estado de tentativa");
        setLoginAttempted(false);
      }
    } catch (error) {
      console.error("Erro durante submissão do login:", error);
      setLoginAttempted(false);
    }
  };
